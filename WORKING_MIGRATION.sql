...
-- Drop functions first (they depend on tables but don't need tables to be dropped)
...
-- Drop tables with CASCADE (this automatically removes triggers, policies, etc.)
...
DO $$
BEGIN
    RAISE NOTICE '✅ Cleanup complete - Starting fresh installation...';
    RAISE NOTICE '';
END $$;
...
CREATE TABLE help_me_pay_requests (
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    CONSTRAINT helper_contact_check CHECK (helper_email IS NOT NULL OR helper_phone IS NOT NULL)
);
...
RETURNS TABLE(
    id UUID, name VARCHAR, description TEXT, price DECIMAL,
    image_url VARCHAR, category_slug VARCHAR, badge VARCHAR,
    stock_quantity INTEGER, is_active BOOLEAN
) AS $$
BEGIN
    RETURN QUERY
    SELECT p.id, p.name, p.description, p.price, p.image_url,
           p.category_slug, p.badge, p.stock_quantity, p.is_active
    FROM products p
...
CREATE FUNCTION create_notification(
    ...
    p_message TEXT, p_metadata JSONB DEFAULT '{}'
)
RETURNS UUID AS $$
DECLARE
...
BEGIN
    ...
    WHERE id = p_notification_id AND (user_id = auth.uid() OR auth.uid() IS NULL);
    RETURN FOUND;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
RETURNS TABLE(
    id UUID, user_id UUID, type VARCHAR, title VARCHAR,
    message TEXT, metadata JSONB, is_read BOOLEAN,
    created_at TIMESTAMP WITH TIME ZONE, updated_at TIMESTAMP WITH TIME ZONE
) AS $$
BEGIN
    RETURN QUERY
    SELECT n.id, n.user_id, n.type, n.title, n.message,
           n.metadata, n.is_read, n.created_at, n.updated_at
...
-- Create help me pay request
CREATE FUNCTION create_help_me_pay_request(
    p_requester_user_id UUID, p_requester_name VARCHAR(255),
    p_requester_email VARCHAR(255), p_requester_phone VARCHAR(50),
    p_helper_name VARCHAR(255), p_helper_email VARCHAR(255),
    p_helper_phone VARCHAR(50), p_helper_message TEXT,
    p_order_data JSONB, p_currency VARCHAR(10), p_total_amount DECIMAL(10, 2)
)
RETURNS UUID AS $$
DECLARE
    request_id UUID;
...
-- Get help me pay request
CREATE FUNCTION get_help_me_pay_request(p_request_id UUID)
RETURNS TABLE(
    id UUID, requester_user_id UUID, requester_name VARCHAR,
    requester_email VARCHAR, requester_phone VARCHAR, helper_name VARCHAR,
    helper_email VARCHAR, helper_phone VARCHAR, helper_message TEXT,
    order_data JSONB, currency VARCHAR, total_amount DECIMAL,
    payment_link VARCHAR, payment_status VARCHAR,
    expires_at TIMESTAMP WITH TIME ZONE, paid_at TIMESTAMP WITH TIME ZONE,
    created_at TIMESTAMP WITH TIME ZONE, updated_at TIMESTAMP WITH TIME ZONE
) AS $$
BEGIN
    RETURN QUERY
    SELECT h.id, h.requester_user_id, h.requester_name, h.requester_email, h.requester_phone,
           h.helper_name, h.helper_email, h.helper_phone, h.helper_message,
           h.order_data, h.currency, h.total_amount, h.payment_link, h.payment_status,
           h.expires_at, h.paid_at, h.created_at, h.updated_at
    FROM help_me_pay_requests h
    WHERE h.id = p_request_id AND (h.expires_at > NOW() OR h.payment_status = 'paid');
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Update help me pay status
CREATE FUNCTION update_help_me_pay_status(
    p_request_id UUID, p_new_status VARCHAR(50), p_order_id UUID DEFAULT NULL
)
RETURNS BOOLEAN AS $$
DECLARE
...
DO $$
BEGIN
    RAISE NOTICE '✅ Functions created (10 total)';
    RAISE NOTICE '';
END $$;
...
DO $$
BEGIN
    RAISE NOTICE '✅ RLS Policies created (7 total)';
    RAISE NOTICE '';
END $$;
