...
ALTER TABLE orders ADD COLUMN IF NOT EXISTS exchange_rate DECIMAL(10, 4) DEFAULT 1.0000;
ALTER TABLE orders ADD COLUMN IF NOT EXISTS payment_type VARCHAR(50) DEFAULT 'direct'; -- 'direct' or 'help_me_pay'
ALTER TABLE orders ADD COLUMN IF NOT EXISTS help_me_pay_request_id UUID REFERENCES help_me_pay_requests(id);
...
GRANT SELECT ON help_me_pay_requests TO authenticated;
...
(
    ...
    'order',
    'Order Confirmed',
    'Your order #ORD-1001 has been confirmed and is being processed.'
),
(
    (SELECT id FROM auth.users LIMIT 1),
    'shipping',
