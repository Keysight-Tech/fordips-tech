-- ═══════════════════════════════════════════════════════════════════════════════
-- 🔍 FORDIPS TECH - PRODUCT DISPLAY VERIFICATION SCRIPT (SUPABASE COMPATIBLE)
-- ═══════════════════════════════════════════════════════════════════════════════
--
...
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '═══════════════════════════════════════════════════════════════';
    RAISE NOTICE '🔍 PRODUCT DISPLAY VERIFICATION - STARTING';
    RAISE NOTICE '═══════════════════════════════════════════════════════════════';
    RAISE NOTICE '';
END $$;
...
BEGIN
    ...
    ELSE
        RAISE WARNING 'WARNING: Only found % products (expected >= 75)', total_products;
    END IF;
    RAISE NOTICE '';
END $$;
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_active_products_by_category') THEN
        ...
        ELSE
            RAISE WARNING 'WARNING: View out of sync by % products', difference;
            RAISE NOTICE 'Fix: Run SELECT refresh_all_product_materialized_views();';
        END IF;
    ELSE
        RAISE WARNING 'WARNING: Materialized view not found';
...
BEGIN
    ...
    ELSE
        RAISE WARNING 'WARNING: Some products have missing or invalid data';
    END IF;
    RAISE NOTICE '';
END $$;
...
BEGIN
    ...
    ELSE
        RAISE WARNING 'WARNING: % empty categories found', empty_categories;
        FOR category_record IN
            ...
            WHERE p.id IS NULL
        LOOP
            RAISE NOTICE 'Empty category: %', category_record.name;
        END LOOP;
    END IF;
    RAISE NOTICE '';
END $$;
...
BEGIN
    ...
    ELSE
        RAISE WARNING 'WARNING: % active products have invalid image URLs', invalid_urls;
    END IF;
    RAISE NOTICE '';
END $$;
...
BEGIN
    ...
    RAISE NOTICE 'Low Stock (1-9): % products', low_stock;
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'search_products_optimized') THEN
        ...
        ELSE
            RAISE WARNING 'WARNING: Search returned no results for iPhone';
        END IF;
    ELSIF EXISTS (SELECT 1 FROM pg_matviews WHERE matviewname = 'mv_product_search_fts') THEN
        ...
        ELSE
            RAISE WARNING 'WARNING: Search view returned no results';
        END IF;
    ELSE
        RAISE WARNING 'WARNING: Search optimization not installed';
        RAISE NOTICE 'Fix: Run SUPABASE_DB_OPTIMIZATION.sql script';
    END IF;
    RAISE NOTICE '';
END $$;
...
BEGIN
    ...
    IF mv_exists THEN
        ...
        ELSE
            RAISE NOTICE 'View Sync Status: NEEDS REFRESH';
        END IF;
    ELSE
        RAISE NOTICE 'Optimization Status: NOT INSTALLED';
    ...
    ELSE
        RAISE NOTICE '═══════════════════════════════════════════════════════════════';
        RAISE NOTICE 'SOME ISSUES DETECTED';
        RAISE NOTICE '═══════════════════════════════════════════════════════════════';
        RAISE NOTICE '';
